...
# Preparación
Necesitamos tener instalados los paquetes `ggplot2` y `qvalue` (qvalue es mejor que la corrección de Bonferroni) en R. En la
carpeta de trabajo encontraréis también los datos necesarios.
#Puede que no podamos descarganos el paquete qvalue. Al final del guion habia un comando para instalarlo, en el caso de que no se pueda lo borramos.Añadimos información a la tabla mediante:
...
#También representamos un histograma para observar si se cumplee la afirmación. En este caso vemos que la distribución es hacia la izquierda, por lo comentado previamente.
...
#Sirve para mejorar la presentación de los gráficos y evitar overplotting.
#Es una opción para los puntos; indica que introduzca un poco de ruido en la posición de las x para evitar que haya "overplotting" y se vea más fácilmente. Evitamos que se representen los puntos unos sobre otros.
...
#Podríamos generar otro marco de datos en el que tuvieramos la media que corresponde a cada grupo; implicaría generar otro marco de datos y unir los datos secundarios en el geom_point. No definimos los datos que estamos usando en los geométricos, sino que utiliza los que se han empleado anteriores. Es más trabajo.
